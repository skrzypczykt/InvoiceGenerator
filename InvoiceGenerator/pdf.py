...
class BaseInvoice(object):
    ...
    def __init__(self, invoice):
        assert isinstance(invoice,
                          Invoice), "invoice is not instance of Invoice"
...
def prepare_invoice_draw(self):
    ...
    if self.invoice.currency:
        warnings.warn(
            "currency attribute is deprecated, use currency_locale instead",
            DeprecationWarning)
...
class SimpleInvoice(BaseInvoice):
    ...
    def _drawAddress(self, top, left, width, height, header_string, address):
        ...
        header = ParagraphStyle('header', fontName='DejaVu', fontSize=12,
                                leading=15)
        default = ParagraphStyle('default', fontName='DejaVu', fontSize=8,
                                 leading=8.5)
        ...
        if hasattr(address, "logo_file") and address.logo_file:
            height = 30.0
            width = 30.0
            self.pdf.drawImage(self.invoice.provider.logo_file,
                               (left + 84) * mm - width, (top - 4) * mm, width,
                               height, mask="auto")

    def _drawClient(self, TOP, LEFT):
        self._drawAddress(TOP, LEFT, 88, 41, _(u'Customer'),
                          self.invoice.client)

    def _drawProvider(self, TOP, LEFT):
        self._drawAddress(TOP, LEFT, 88, 36, _(u'Provider'),
                          self.invoice.provider)

    def _drawPayment(self, TOP, LEFT):
        self.pdf.setFont('DejaVu-Bold', 8)
        self.pdf.drawString(LEFT * mm, (TOP + 2) * mm,
                            _(u'Payment information') + ": przelew")

        text = self.pdf.beginText((LEFT) * mm, (TOP - 2) * mm)
        lines = [
            self.invoice.provider.bank_name,
            '%s: %s' % (
                _(u'Account n.'), self.invoice.provider.bank_account_str()),
        ]
        if self.invoice.variable_symbol:
            lines.append(
                '%s: %s' % (
                    _(u'Variable symbol'), self.invoice.variable_symbol),
            )
    ...
    def _drawItems(self, TOP, LEFT):  # noqa
        ...
        for item in self.invoice.items:
            ...
            pwidth, pheight = p.wrapOn(self.pdf,
                                       70 * mm if items_are_with_tax else 90 * mm,
                                       30 * mm)
            ...
            if will_wrap and TOP - i - i_add < 8 * mm:
                ...
                self.pdf.rect(LEFT * mm, (TOP - i) * mm, (LEFT + 156) * mm,
                              (i + 2) * mm, stroke=True, fill=False)  # 140,142
                self.pdf.showPage()
            ...
            if items_are_with_tax:
                ...
                self.pdf.drawRightString((LEFT + 134) * mm, (TOP - i) * mm,
                                         currency(item.total,
                                                  self.invoice.currency,
                                                  self.invoice.currency_locale))
                self.pdf.drawRightString((LEFT + 144) * mm, (TOP - i) * mm,
                                         '%.0f %%' % item.tax)
                self.pdf.drawRightString((LEFT + 173) * mm, (TOP - i) * mm,
                                         currency(item.total_tax,
                                                  self.invoice.currency,
                                                  self.invoice.currency_locale))
                i += 5
            else:
                if float(int(item.count)) == item.count:
                    self.pdf.drawRightString((LEFT + 100) * mm, (TOP - i) * mm,
                                             u'%s' % (
                                                 locale.format_string("%i",
                                                                      item.count,
                                                                      grouping=True)))
        ...
        if not items_are_with_tax:
            ...
            self.pdf.drawString((LEFT + 1) * mm, (TOP - i - 2) * mm,
                                _(u'Breakdown VAT'))
            vat_list, tax_list, total_list, total_tax_list = [_(u'VAT rate')], [
            ...
            self.pdf.drawString(
                ...
                u'%s: %s' % (_(u'Total with tax'),
                             currency(self.invoice.price_tax,
                                      self.invoice.currency,
                                      self.invoice.currency_locale)),
            )
        ...
        if items_are_with_tax:
            self.pdf.rect(LEFT * mm, (TOP - i - 17) * mm, (LEFT + 156) * mm,
                          (i + 19) * mm, stroke=True, fill=False)  # 140,142
        else:
            self.pdf.rect(LEFT * mm, (TOP - i - 11) * mm, (LEFT + 156) * mm,
                          (i + 13) * mm, stroke=True, fill=False)  # 140,142
    ...
    def _drawCreator(self, TOP, LEFT):
        ...
        if self.invoice.creator.stamp_filename:
            ...
            self.pdf.drawImage(self.invoice.creator.stamp_filename, (LEFT) * mm,
                               (TOP - 2) * mm - height, 200, height,
                               mask="auto")
        ...
        self.pdf.drawString((LEFT + 10) * mm, (TOP - 5) * mm - height,
                            '%s: %s' % (
                                _(u'Creator'), self.invoice.creator.name))
        ...
        self.pdf.drawString((30) * mm, (TOP - 5) * mm - height,
                            '%s' % (_(u'Customer')))
    ...
    def _drawDates(self, TOP, LEFT):
        ...
        if self.invoice.date and self.invoice.use_tax:
            ...
            items.append((LEFT * mm, '%s: %s' % (_(u'Date of exposure'),
                                                 format_date(self.invoice.date,
                                                             locale=lang))))
        ...
        if self.invoice.payback:
            items.append((LEFT * mm, '%s: %s' % (
                _(u'Due date'),
                format_date(self.invoice.payback, locale=lang))))
...
class CorrectingInvoice(SimpleInvoice):
    ...
    def drawCorretion(self, TOP, LEFT):
        ...
        self.pdf.drawString(LEFT * mm, TOP * mm,
                            _(u'Correction document for invoice: %s') % self.invoice.number)
        self.pdf.drawString(LEFT * mm, (TOP - 4) * mm,
...
class ProformaInvoice(SimpleInvoice):
    ...
    def _drawDates(self, TOP, LEFT):
        ...
        if self.invoice.date:
            items.append((LEFT * mm, '%s: %s' % (
                _(u'Date of exposure'), self.invoice.date)))
        if self.invoice.payback:
            items.append(
